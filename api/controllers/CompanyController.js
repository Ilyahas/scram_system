...
class CompanyController extends BaseController {
  async getListOfUnverifiedCompanies(req, res, next) {
    try {
      let unverifiedCompanies = await Company.find({ confirmed: false })
        .populate({
          path: 'owner',
          model: 'User',
  ...
  async getCompany(req, res, next) {
    try {
      let company = await Company.findOne({ owner: req.user }, '-workers -confirmed')
        .populate({
          path: 'listOfTeams',
          populate: {
  ...
  async approveCompanies(req, res, next) {
    try {
      ...
      let approvedCompanies = await Company.update(criteria, { $set: { confirmed: true } }, { multi: true })
